...
function form_featured() { 
	if (isset($_POST['doc_name'])) { 
            ...
            */
            ...
            if ( empty( $doc_name ) || empty( $phone ) || empty( $add1) || empty( $city ) || empty( $state ) || empty( $zip ) || empty($website) || empty($description) || empty($l_name) || empty($f_name) || empty($email) ) {
                $reg_errors->add('field', 'Required form field is missing');
            }
            if(isset($_POST['upload_img_id'])){
                $upload_img_id= sanitize_text_field($_POST['upload_img_id']);
            } else {
                if ( 
                        isset( $_POST['my_image_upload_nonce'] ) 
                        && wp_verify_nonce( $_POST['my_image_upload_nonce'], 'my_image_upload' )
                ) { 
                        if( $_FILES["my_image_upload"]['size'] == 0 ) { 
                            $reg_errors->add('image', 'Please upload image');
                        } else { 
                            // The nonce was valid and the user has the capabilities, it is safe to continue.

                            //check height and width of image for virus protection

                            list($width, $height) = getimagesize($_FILES["my_image_upload"]['tmp_name']);
                            if( $width > 0 && $height > 0)
                            { 
                                ...
                                if ( is_wp_error( $attachment_id ) ) {
                                    ...
                                    $reg_errors->add('image', 'There was an error uploading the image');
                                } else {
                                    $fullsizepath = get_attached_file( $attachment_id );
                                    wp_update_attachment_metadata( $attachment_id, wp_generate_attachment_metadata( $attachment_id, $fullsizepath ) );
                                    $upload_img_id = $attachment_id;
                                }
                            } else {
                                $reg_errors->add('image', 'Please upload only JPGE/PNG image file');
                            }
                        }    
                } else {
                    // The security check failed, maybe show the user an error.
                    $reg_errors->add('image', 'The security check failed');
                }
            }
	} 
?>
<form class="my-frm" method="post" id="payment-form" action="<?php echo get_home_url();?>/directory-subscription/?type=featured" enctype="multipart/form-data">
<div class="container-my">
	<div class="package-frm">
	<div class="row-my">
	<div class="col-12-my m-10">
		<h3 class="frm-top-hding" style="margin-top:20px;">Add a Featured Listing</h3><hr class="divider-line">
		<?php
                if (isset($_POST['doc_name'])) {
                    ...
                    if(count($reg_errors->errors) == 0) {
                            $attachment_id =$upload_img_id;
                            $img = wp_get_attachment_image_src( $attachment_id, 'dir-img');
                            $thanks_html = '
                                    </div>
                                    <div class="col-12-my">
                                    ...
                                    <div class="col-12-my">
                                        <p>Description:&nbsp;&nbsp;<br><b>'.$description.'</b></p>
                                    </div>
                                    <div class="col-12-my">
                                        <p>Image:&nbsp;&nbsp;<b><br><img src="'.$img[0].'"></b></p>
                                    </div>
                                    <div class="col-12-my">
                                        <h3 class="sub-hding">Order Summary</h3>
                                        <p><b>Featured Listing Monthly Subscription</b><br>$69.00 today, then $69.00/month<br>Your subscription will continue until cancelled.<br><br>Total: $69.00</p>
                                    </div>
                                    <div class="col-12-my">
                                        <h3 class="sub-hding">Payment Information</h3>
        ...
        <div class="row-my">
            ...
            <div class="col-6-my">
                <div class="frm-grp">
                    ...
                    <input class="form-control <?php echo ( isset($_POST['email']) && empty($_POST['email']) )? "error": '';?>"  oninput="setCustomValidity('')" oninvalid="this.setCustomValidity('Enter an email id')" required="" type="email" name="email" value="<?php echo ( isset($_POST['email']) && empty($_POST['email']) )? "": $_POST['email'];?>">
            ...
            <div class="col-6-my">
                 <div class="frm-grp">
                    <label for="phone no">Phone (optional)</label>
                        <input class="form-control" type="tel" name="c_phone" value="<?php echo ( isset($_POST['c_phone']) && empty($_POST['c_phone']) )? "": $_POST['c_phone'];?>">
                </div>
            </div>
            <div class="col-12-my">
                <h3 class="sub-hding">Listing Information</h3>
            ...
            <div class="col-6-my">
                <div class="frm-grp">
                    ...
                    <input class="form-control <?php echo ( isset($_POST['phone']) && empty($_POST['phone']) )? "error": '';?>" oninput="setCustomValidity('')"  oninvalid="this.setCustomValidity('Enter Phone No.')"  required="" type="tel" name="phone" value="<?php echo ( isset($_POST['phone']) && empty($_POST['phone']) )? "": $_POST['phone'];?>">
        ...
        <div class="col-6-my">
            <div class="frm-grp">
                ...
                <span class="error">Please enter valid website url</span>
                <?php } ?>
            </div>
        </div>
        <div class="col-12-my">
        ...
        <div class="col-12-my">
            <div class="frm-grp">
                ...
                <?php } else { ?>
                    ...
                    <input class="form-control" type="file" name="my_image_upload" multiple="false"  oninput="setCustomValidity('')"  oninvalid="this.setCustomValidity('upload image')"  required="">
                    ...
                    <?php if(count($reg_errors->errors) > 0 && !empty($reg_errors->errors[image][0])) { ?>
