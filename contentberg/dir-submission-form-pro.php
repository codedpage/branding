 ...
 */
        ...
        if (isset($_POST['doc_name'])) { 
            ...
            $level_of_cares = $_POST['level_of_cares'] ;
            ...
            if ( empty($level_of_cares) ||  empty( $doc_name ) || empty( $phone ) || empty( $add1) || empty( $city ) || empty( $state ) || empty( $zip ) || empty($l_name) || empty($f_name) || empty($email) ) {
                    $reg_errors->add('field', 'Required form field is missing');
            }
            
            if(isset($_POST['cov_img_id'])){
                $cov_img_id= sanitize_text_field($_POST['cov_img_id']);
            } else {
                if ( 
                        isset( $_POST['my_image_upload_nonce'] ) 
                        && wp_verify_nonce( $_POST['my_image_upload_nonce'], 'my_image_upload' )
                ) { 
                        if( $_FILES["cov_img_upload"]['size'] == 0 ) { 
                            ...
                            list($width, $height) = getimagesize($_FILES["cov_img_upload"]['tmp_name']);
                            if( $width > 0 && $height > 0)
                            { 
                                ...
                                $attachment_id = media_handle_upload( 'cov_img_upload', 0 );
            ...
            if(isset($_POST['gallery_image_ids'])){
                foreach($_POST['gallery_image_ids'] as $k=>$v)
                {
                    $gallery_image_ids[] = $v;
                }
}
                ...
                if ( 
                        ...
                        if( empty($_FILES["gallery_upload"]['size'])) { 
                            $reg_errors->add('image', 'Please upload image');
                        } else { 
                            // These files need to be included as dependencies when on the front end.
                            require_once( ABSPATH . 'wp-admin/includes/image.php' );
                            ...
                            foreach($files as $key=>$v)
                            {
                                if ($files['name'][$key]) { 
                                    ...
                                    foreach ($_FILES as $file => $array) {              
                                        $galler_img_ids[] = my_handle_attachment($file,$pid); 
                                    }
                                }
                                // The nonce was valid and the user has the capabilities, it is safe to continue.
...
?>
<form class="my-frm" method="post" id="payment-form" action="<?php echo get_home_url();?>/directory-subscription" enctype="multipart/form-data">
<div class="container-my">
    <input type="hidden" name="selected_option" value="pro">
	<div class="package-frm">
	<div class="row-my">
	<div class="col-12-my m-10">
		<h3 class="frm-top-hding" style="margin-top:20px;">Add a Pro Listing</h3><hr class="divider-line">
		<?php
                if (isset($_POST['doc_name'])) {
...
s                   </select>
        ...
        <div class="col-12-my">
            <div class="frm-grp">
                ...
                <?php if(isset($cov_img_id) && $cov_img_id != null) { 
                    $img = wp_get_attachment_image_src( $cov_img_id, 'thumbnail');
                ?>
                <img src="<?php echo $img[0];?>"><input type="hidden" name="cov_img_id" value="<?php echo $cov_img_id;?>">
                <?php } else { ?>
                    <?php wp_nonce_field( 'my_image_upload', 'my_image_upload_nonce' ); ?>
                    <input class="form-control" type="file" name="cov_img_upload" id="my_image_upload" multiple="false"  oninput="setCustomValidity('')"  oninvalid="this.setCustomValidity('upload image')"  required="">
        ...
        <div class="col-12-my">
            <div class="frm-grp">
                ...
                <?php if(!empty($gallery_image_ids)) { 
                    foreach($gallery_image_ids as $k=>$v)
                    {
                        $img = wp_get_attachment_image_src( $v, 'thumbnail');
                        ?>
                        <img src="<?php echo $img[0];?>"><input type="hidden" name="gallery_image_ids[]" value="<?php echo $v;?>">
                        <?php
                    }
                ?>
                <?php } else { ?>
